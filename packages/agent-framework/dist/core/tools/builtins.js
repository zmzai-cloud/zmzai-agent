...
export const readTool = {
    ...
    async execute(args, ctx) {
        ...
        if (!file)
            throw new Error(`文件不存在或不可读取：${args.path}`);
...
export const writeTool = {
    ...
    async execute(args, ctx) {
        ...
        if (!result)
            throw new Error(`路径不合法或写入被拒绝：${args.path}`);
...
export const editTool = {
    ...
    async execute(args, ctx) {
        ...
        if ("error" in result)
            throw new Error(`编辑失败：${result.error}`);
        await ctx.emitFileEdited({ path: args.path, revisionId: result.revisionId, diff: result.diff });
...
function allowedPrograms() {
    const configured = process.env.EXEC_ALLOWED_PROGRAMS?.trim();
    if (!configured)
        return new Set(defaultAllowedPrograms);
...
function splitProgram(program) {
    ...
    if (/[|><;&]/.test(trimmed))
        return { program: "sh", args: ["-c", trimmed] };
    const tokens = [];
    const pattern = /"([^"]*)"|'([^']*)'|(\S+)/g;
    let match;
...
export const bashTool = {
    id: "bash",
    label: "在沙箱中执行命令",
    description: "在当前 Workspace 的快照中执行一条命令（隔离沙箱）。程序必须在允许列表内；stdout/stderr 会返回。新生成的产物文件会自动出现在用户的产物面板（可预览/下载）——回复中不要自己编造下载链接（如 sandbox: 伪协议或沙箱路径），引用产物时使用工具结果里给出的真实 URL。",
    parameters: z.object({
    ...
    async execute(args, ctx) {
        ...
        if (!allowedPrograms().has(program))
            throw new Error(`程序 "${program}" 不在允许列表`);
        ...
        for (const artifact of result.artifacts) {
            if (artifact.workspaceContent !== undefined) {
                ...
                if (written)
                    await ctx.emitFileEdited({ path: artifact.path, revisionId: written.revisionId, diff: written.diff });
            }
...
export const builtinTools = [readTool, globTool, grepTool, writeTool, editTool, todoTool, bashTool, qaCheckTool, webfetchTool, taskTool];
