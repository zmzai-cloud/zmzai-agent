...
export function createSubprocessSandbox(input) {
    const wsRoot = () => (typeof input?.workspaceRoot === "function" ? input.workspaceRoot() : input?.workspaceRoot) ?? null;
    return {
        async buildSnapshot() {
            const ws = wsRoot();
            if (!ws)
            ...
            const walk = async (dir) => {
                if (files.length >= SNAPSHOT_MAX_FILES || total >= SNAPSHOT_MAX_BYTES)
                    return;
                ...
                const walk = async (dirPath) => {
                    ...
                    for (const entry of entries) {
                        ...
                        if (entry.isDirectory()) {
                            ...
                            if (ARTIFACT_SKIP_DIRS.has(rel.split(path.sep)[0] ?? ""))
                                continue;
...
//# sourceMappingURL=subprocess-sandbox.js.map