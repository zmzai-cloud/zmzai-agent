...
function sessionIdOf(event, fallbackSessionId) {
    if (event.type === "message.updated")
        return event.data.message.sessionId;
    if (event.type === "message.part.updated")
        return event.data.part.sessionId;
...
export class SessionRunner {
    ...
    get hooks() {
        if (this.#hooks.length !== (this.deps.hooks?.length ?? 0))
    ...
    async persist(event, fallbackSessionId) {
        if (event.type === "message.updated") {
            ...
            if (exists)
                await this.deps.store.updateMessage(message.id, message);
            else
                await this.deps.store.appendMessage(message);
        }
    ...
    async runLoop(session, input) {
        ...
        if (next) {
            ...
            if (latest)
                await this.runLoop(latest, { text: next.text, ...(next.agent ? { agent: next.agent } : {}), ...(next.effort ? { effort: next.effort } : {}) });
        }
    }
    ...
    async recordSubtask(parent, input) {
    ...
    async rebuildMessages(sessionId) {
        const entries = await this.deps.store.getMessages(sessionId);
        const messages = [];
