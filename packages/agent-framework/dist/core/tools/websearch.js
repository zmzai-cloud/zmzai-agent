...
async function searchViaTavily(query, apiKey, maxResults, doFetch) {
...
async function searchViaSerper(query, apiKey, maxResults, doFetch) {
...
async function searchViaDuckDuckGo(query, maxResults, doFetch) {
...
export function createWebSearchTool(opts = {}) {
    ...
    const envLoader = opts.envLoader ?? (() => ({ TAVILY_API_KEY: process.env.TAVILY_API_KEY, SERPER_API_KEY: process.env.SERPER_API_KEY }));
    return {
        id: "websearch",
        label: "联网搜索",
        description: "搜索互联网并返回标题+链接+摘要列表（多数结果无需再抓取即可判断相关性；需要全文时对结果 URL 用 webfetch）。查询用与目标资料相同的语言效果最好。",
        parameters: z.object({
