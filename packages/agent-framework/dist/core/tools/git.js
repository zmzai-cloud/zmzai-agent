...
function runGit(cwd, args, timeoutMs) {
    return new Promise((resolvePromise, reject) => {
        execFile("git", args, { cwd, timeout: timeoutMs, maxBuffer: 16 * 1024 * 1024, windowsHide: true }, (error, stdout, stderr) => {
            if (!error) {
...
export function createGitTools(options) {
    ...
    const gitStatusTool = {
        ...
        async execute() {
            ...
            const entries = lines.slice(1).map(parseStatusLine).filter((e) => e !== null);
            const staged = entries.filter((e) => e.x !== " " && e.x !== "?");
            const unstaged = entries.filter((e) => e.y !== " " && e.y !== "?");
            const untracked = entries.filter((e) => e.x === "?" && e.y === "?");
            const format = (list) => list.map((e) => (e.origPath ? `${e.path}（自 ${e.origPath} 重命名/复制）` : e.path)).join("\n") || "（无）";
            const dirtyNote = entries.length === 0 ? "\n工作区是干净的。" : "";
            const output = `分支 ${branch}${ahead ? ` · 领先远端 ${ahead} 个提交` : ""}${behind ? ` · 落后远端 ${behind} 个提交` : ""}\n` +
                `已暂存（${staged.length}）:\n${format(staged)}\n` +
    ...
    const gitLogTool = {
        ...
        async execute(args) {
            ...
            const run = await runGit(cwdOf(), ["log", `--max-count=${limit}`, "--pretty=format:%h%x09%an%x09%aI%x09%s"], 15_000);
    ...
    const gitCommitTool = {
        id: "git_commit",
        label: "创建 Git 提交",
        description: "把指定文件（paths）或全部变更（addAll: true）暂存并创建提交；都不传则直接提交当前已暂存的内容。message 必填。提交真实发生在仓库中。",
        parameters: z.object({
        ...
        async execute(args) {
            ...
            if (!stagedFiles.length) {
                throw new Error("没有可提交的变更：工作区既没有已暂存内容，也没有匹配到待暂存文件。先用 git_status 确认改动，再通过 paths 或 addAll 指定要提交的内容。");
            }
