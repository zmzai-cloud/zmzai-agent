...
export function pruneOutput(text) {
...
export function adaptTool(def, ctx) {
    ...
    return {
        ...
        prepareArguments: (rawParams) => repairToolArguments(rawParams),
        async execute(toolCallId, rawParams) {
            ...
            return {
                content: [{ type: "text", text: pruned.text }],
                details: {
                    title: result.title,
                    ...(result.metadata ?? {}),
...
export function adaptExternalTool(def, ctx) {
    return {
        name: def.id,
        label: def.label,
        description: def.description,
        parameters: structuredClone(def.parametersJsonSchema),
        ...(def.executionMode ? { executionMode: def.executionMode } : {}),
        prepareArguments: (rawParams) => repairToolArguments(rawParams),
        async execute(toolCallId, rawParams) {
            const args = repairToolArguments(rawParams);
            if (!isRecord(args))
                throw new Error("参数无效：必须是 JSON 对象，请修正后重新调用");
            let result;
            try {
                result = await def.execute(args, { ...ctx, toolCallId });
            }
            catch (error) {
                const message = error instanceof Error ? error.message : String(error);
                throw new Error(message.length > 8_000 ? trimFailureOutput(message).text : message);
...
export function adaptAnyTool(def, ctx) {
 ...
 *  beforeToolCall hook. */
export function permissionForCall(defs, toolName, args) {
    const def = defs.get(toolName);
    if (!def)
        return null;
    if (isExternalToolDef(def)) {
        return isRecord(args) ? def.permission(args) : null;
    }
    const parsed = def.parameters.safeParse(args);
    if (!parsed.success)
        return null; // invalid args fail in execute() with a clear message
