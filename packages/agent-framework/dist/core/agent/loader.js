...
function assign(data, key, value) {
...
function toAgentInfo(fileName, parsed) {
    const { data, body } = parsed;
    const name = data.name ?? fileName.replace(/\.md$/i, "");
    if (!data.description)
        return { error: `agent ${fileName} 缺少必填的 description` };
    const mode = data.mode && VALID_MODES.has(data.mode) ? data.mode : "primary";
    if (data.mode && !VALID_MODES.has(data.mode))
        return { error: `agent ${fileName} 的 mode 必须是 primary|subagent|all` };
    let permission = [];
    if (data.permission) {
        const config = {};
...
function parseModel(value) {
    ...
    if (idx === -1)
        return { providerId: "relay", modelId: value };
...
export async function loadCustomAgents(workspace) {
