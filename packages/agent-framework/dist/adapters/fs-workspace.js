...
function revFile(root) {
    return path.join(root, ".fw-revisions.json");
}
async function loadRevisions(root) {
    try {
        return JSON.parse(await readFile(revFile(root), "utf8"));
    }
    catch {
        return [];
    }
}
async function saveRevisions(root, revisions) {
...
export function createFsWorkspaceFiles(input) {
    ...
    return {
        async list() {
            ...
            const walk = async (dir) => {
                ...
                for (const entry of entries) {
                    if (entry.name === ".fw-revisions.json" || entry.name.startsWith(".git"))
                        continue;
        ...
        async edit({ path: rel, oldText, newText, summary }) {
            ...
            if (first === -1)
                return { error: "EDIT_TARGET_NOT_FOUND" };
            if (content.indexOf(oldText, first + oldText.length) !== -1)
                return { error: "EDIT_TARGET_AMBIGUOUS" };
...
//# sourceMappingURL=fs-workspace.js.map