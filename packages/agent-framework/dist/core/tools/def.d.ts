...
export type ToolDef<TSchema extends z.ZodType = z.ZodType<any>> = {
    ...
    permission: (args: z.output<TSchema>) => {
        permission: string;
        patterns: string[];
        always?: string[];
        metadata?: unknown;
    } | null;
    execute(args: z.output<TSchema>, ctx: ToolContext): Promise<{
        title: string;
        output: string;
...
export type ExternalToolDef = {
    ...
    permission: (args: Record<string, unknown>) => {
        permission: string;
        patterns: string[];
        always?: string[];
        metadata?: unknown;
...
export declare function isExternalToolDef(def: AnyToolDef): def is ExternalToolDef;
