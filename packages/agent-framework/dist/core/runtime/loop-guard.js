...
export function stormSignature(toolName, errorText) {
    return `${toolName}\u0000${normalizeError(errorText)}`;
}
/** edit 重复失败的语义签名：(path, oldText)。 */
export function editFailureSignature(path, oldText) {
    return `${path}\u0000${oldText}`;
}
export function strategyAdvisory(toolName, kind) {
    const what = kind === "failure" ? "连续以相同的响应失败" : "连续被权限拒绝";
    return (`[循环防护] ${toolName} 已经${what}多次。换措辞重发没有用：调用会继续命中同样的结果。` +
...
export class LoopGuard {
    ...
    onToolResult(input) {
        ...
        if (signature === this.stormSignature)
            this.stormCount += 1;
        else {
