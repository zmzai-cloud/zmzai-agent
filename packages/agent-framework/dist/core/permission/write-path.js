...
import { wildcardMatch } from "./ruleset.js";
/** 路径是否落在写路径白名单内：等于白名单根，或位于其子树。 */
export function pathInWritePaths(p, writePaths) {
...
export function writePathGuardRules(writePaths) {
    if (writePaths.length === 0)
        return [];
    return [
        ...writePaths.map((root) => ({ permission: "edit", pattern: `${root}/**`, action: "allow" })),
        { permission: "edit", pattern: "**", action: "deny" },
    ];
}
/** 结构层圈禁：包住 workspace 门面，越界 write/edit 直接抛错（不可绕过）。
 *  read/list 保持原样——探索型子代理需要全库视野，只圈写。 */
export function confineWorkspaceFiles(workspace, writePaths) {
    const guard = (p) => {
...
//# sourceMappingURL=write-path.js.map