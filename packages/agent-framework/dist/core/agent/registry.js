...
const DEFAULT_PROMPT = "你是 ZMZAI Agent，一个会主动完成任务的 Coding Agent。可使用工具读取当前 Workspace，通过 write/edit 直接修改文件（每次修改生成可审查、可回滚的不可变版本），用 bash 在隔离沙箱中运行命令（stdout/stderr 会返回，生成的产物文件可下载）。\n" +
...
const READONLY_PROMPT = "你是 ZMZAI Agent 的只读分析模式。仅使用 read/glob/grep 工具读取当前 Workspace 并给出可核实的中文方案；你没有写入和执行权限，不能声称执行了任何修改。任务结束后用户可以在同一会话继续追问，或切换到 default 代理执行你的方案。";
const EXPLORE_PROMPT = "你是代码库探索子代理。快速定位与任务相关的文件、符号与数据流，返回精确的 path:line 引用和简洁结论。你只读，不做任何修改。按要求的彻底程度（quick/medium/thorough）控制搜索范围。";
...
export const builtinDefaults = rulesetFromConfig({
    "*": "allow",
...
export class AgentRegistry {
    ...
    constructor(customAgents = []) {
        ...
        for (const agent of customAgents)
            this.agents.set(agent.name, agent); // custom overrides builtin (spec §6.3)
    }
    get(name) {
    ...
    list(filter) {
        return [...this.agents.values()].filter((agent) => {
            if (!filter?.includeHidden && agent.hidden)
                return false;
