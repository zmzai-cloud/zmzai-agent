...
function ringAppend(ring, chunk, capBytes) {
...
export class TerminalManager {
    ...
    async start(input) {
        ...
        if (this.#sessions.size >= this.#maxSessions) {
            ...
            if (exitedId)
                this.#sessions.delete(exitedId);
            else
                throw new Error(`终端会话已达上限 ${this.#maxSessions}：先 terminal_kill 或等现有进程退出`);
        }
        ...
        const handle = await this.#backend.start({ command: input.command, cwd: input.cwd, ...(input.env ? { env: input.env } : {}), ...(input.cols ? { cols: input.cols } : {}), ...(input.rows ? { rows: input.rows } : {}) }, {
            onData: (chunk) => {
                const buf = Buffer.from(chunk, "utf8");
                session.info.bytesTotal += buf.length;
                ringAppend(session.ring, buf, this.#ringCapBytes);
                for (const wake of session.waiters.splice(0))
                    wake();
            },
    ...
    write(id, text) {
        const s = this.#sessions.get(id);
        if (!s?.handle || s.info.status !== "running")
            return false;
        s.handle.write(text);
        return true;
    }
    resize(id, cols, rows) {
        const s = this.#sessions.get(id);
        if (!s?.handle?.resize || s.info.status !== "running")
            return false;
...
// ---------------------------------------------------------------------------
// 工具集
export function createTerminalTools(manager, opts) {
    const requireSession = (id) => manager.getSession(id) ?? (() => { throw new Error(`终端会话不存在：${id}（用 terminal_start 创建，或 terminal_list 查看）`); })();
    const formatInfo = (s) => `[${s.id}]${s.name ? `「${s.name}」` : ""} ${s.status}${s.exitCode != null ? `(exit ${s.exitCode})` : ""}${s.signal ? `(sig ${s.signal})` : ""} · ${s.backend} · 输出累计 ${s.bytesTotal}B`;
    const startTool = {
        id: "terminal_start",
        label: "启动交互式终端",
        description: "在本机启动一个长驻/交互式命令（dev server、watch、交互安装器等），立即返回 sessionId 不阻塞对话。输出用 terminal_read 增量读取；交互应答用 terminal_write；结束用 terminal_kill。",
        parameters: z.object({
        ...
        async execute(args) {
            ...
            if (!resolvedCwd)
                throw new Error(`cwd 必须位于工作区内：${args.cwd}`);
    ...
    const readTool = {
        id: "terminal_read",
        label: "读取终端输出",
        description: "增量读取指定终端自 since 字节游标以来的输出；返回新的游标（下次传回即可续读）。输出的末尾包含会话状态；进程已退出时会注明退出码，无需继续轮询。",
        parameters: z.object({
        ...
        async execute(args) {
            ...
            if (!result)
                throw new Error(`终端会话不存在：${args.sessionId}`);
            const tailNote = result.session.status === "running"
    ...
    const writeTool = {
        ...
        async execute(args) {
            ...
            if (!ok)
                throw new Error(`无法写入：${args.sessionId} 不是运行中的会话`);
    ...
    const killTool = {
        ...
        async execute(args) {
            ...
            if (!ok && final?.status === "running")
                throw new Error(`终止失败：${args.sessionId} 仍在运行`);
            return {
