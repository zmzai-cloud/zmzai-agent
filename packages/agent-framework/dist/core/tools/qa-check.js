...
export const qaCheckTool = {
    ...
    async execute(args, ctx) {
        ...
        const cssFiles = (await Promise.all((await ctx.workspace.list()).filter((file) => file.path.endsWith(".css")).slice(0, 20).map((file) => ctx.workspace.read(file.path)))).filter((file) => Boolean(file)).map((file) => file.content).join("\n");
        ...
        const missingText = args.requiredText.filter((text) => !html.includes(text));
