...
import { emailConnector } from "@/lib/ipaas/email-connector";
...
export async function POST(request: Request) {
  ...
  if (!connectorId || !to || !text) {
    return apiError("IPAAS_MISSING_FIELDS", 400, "缺少必填字段: connectorId, to, text");
  }

  const connector = await IpaasConnectorModel.findOne({ connectorId, platform: "email" }).select("+encryptedCredentials").lean();
  if (!connector) return apiError("IPAAS_CONNECTOR_NOT_FOUND", 404, "邮件连接器不存在");
  if (connector.status !== "active") return apiError("IPAAS_CONNECTOR_PAUSED", 403, "邮件连接器已暂停");
  ...
  if (!result.success) return apiError("IPAAS_SEND_FAILED", 502, result.error ?? "邮件发送失败");
